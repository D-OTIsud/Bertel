...
\echo '== 17c-test garde permanente chantier 6, prouvee NON VACANTE par sabotage (restriction retiree ⇒ D2 rouge). A structure + REVOKE PUBLIC/anon / B les 3 chemins d autorisation ET les 4 motifs de refus, chacun sur un temoin fabrique — un rang admin SUFFIT sans write_crm_notes, sans quoi le compte du PO lui-meme serait exclu / C non-nullite hors contexte HTTP, 204 : NULL ferait sauter le IF NOT de save_crm_task, garde fail-OPEN / D inclusion liste ⊆ garde / E refus EFFECTIF par save_crm_task — c est la seule assertion qui prouve que la garde est CABLEE, 213 — ET passage toujours possible du chemin autorise, car une garde qui coupe tout le monde satisferait le refus en cassant le produit / F une assignation historique SURVIT : c est une donnee, pas une autorisation courante =='
...
\echo '== NOTE MANIFESTE (revue 2026-09-04) : les trois etapes §227 qui suivent (17i, 17j, 17k) etaient APPLIQUEES EN PRODUCTION depuis le 2026-08-31 mais ABSENTES de ce manifeste — une base fraiche ne portait donc AUCUNE des regles d autorisation qu elles installent, et la garde CI ne les appliquait jamais. Instance de la classe « Deploy integrity (no PROD-only DDL) » de CLAUDE.md. Etat vif verifie le 2026-09-04 AVANT de les declarer — on ne pose pas au manifeste un fichier dont on n a pas prouve qu il decrit le vif : md5(prosrc) compare au corps extrait du fichier est IDENTIQUE pour user_has_permission, rpc_set_role_permission, rpc_list_role_permissions, rpc_list_org_members, seed_org_role_permission, user_can_write_crm, user_can_write_crm_actor, current_user_can_write_crm_notes et create_list ; user_can_write_list ne diverge que par TROIS lignes de COMMENTAIRE ajoutees au fichier apres le deploiement (parite exacte une fois les commentaires retires : 411 caracteres, md5 9c51f7dedfa9d6e7af4d4795ce26d690 des deux cotes). Elles sont posees ICI, en fin de chaine, et non a leur place chronologique : chacune remplace une fonction qu un fichier PLUS BAS du manifeste redefinit, et 17j patche une fonction SUR PLACE — remonter l une d elles la ferait ecraser par un fichier posterieur. Elles doivent rester AVANT 18a, dont la reecriture generique des policies de lecture doit voir toutes les policies deja creees =='

\echo '== 17i    migration_role_permission_matrix.sql  (§227 : le role metier CONFERE les droits, regles par ORG. AJOUT de org_role_permission (ORG x role metier x permission) et d un chemin « role » dans api.user_has_permission ; RETRAIT du chemin org_permission, qui accordait a TOUS les membres de l ORG sans regarder leur role ; user_permission reste, lu desormais pour ce qu il est : des EXCEPTIONS individuelles. GARDE PRE-VOL : la migration REFUSE de s appliquer si org_permission porte encore une ligne active — sur base fraiche la table est VIDE, le seul INSERT du corpus etant loge dans le CORPS de api.rpc_grant_org_permission (rls_policies.sql), donc jamais execute au DDL. Seed du prereglage SP-2 pour chaque ORG existante PLUS trigger trg_seed_org_role_permission sur les ORG creees ensuite : sans lui une ORG neuve naitrait avec une matrice VIDE et ses Editeurs porteraient l etiquette sans aucun droit — panne muette et differee. DROP de rpc_grant_org_permission / rpc_revoke_org_permission, definies en rls_policies.sql ; la TABLE org_permission est CONSERVEE pour la tracabilite de l incident du 31/08. PLACE DICTEE PAR api.rpc_list_org_members, que cette migration recree : APRES 17d migration_team_roster_provenance.sql, dernier AUTRE definisseur du manifeste — la parite md5 prosrc/fichier sur la base vive confirme que 17i en est bien le dernier ecrivain en production. Policy de lecture ecrite en forme initplan (SELECT auth.uid()), conforme a la garde permanente 16h. AUCUN fichier de test versionne : eprouvee en production par sabotage en transaction annulee, cf. docs/SQL_ROLLOUT_RUNBOOK.md 17i) =='
\ir migration_role_permission_matrix.sql

\echo '== 17j    migration_crm_write_requires_permission.sql  (§227 : ecrire du CRM exige la PERMISSION write_crm_notes, jamais le seul rang d administration. Retire le bras api.current_user_admin_rank() IS NOT NULL des QUATRE gardes d ecriture CRM — user_can_write_crm, user_can_write_crm_actor, current_user_can_write_crm_notes, et la branche « acteur en projet » de save_crm_actor. Le test retire n etait pas un SEUIL mais une NON-NULLITE : team_lead vaut 10, tres en dessous du rang 30 exige pour ecrire une permission, si bien que N IMPORTE QUEL role d administration ouvrait l ecriture CRM par-dessus le role metier — deux systemes d autorisation concurrents dont l un ignorait l autre. LA PLACE DE CETTE ETAPE EST DICTEE PAR SON PATCH SUR PLACE : save_crm_actor fait ~200 lignes pour un seul bras a corriger, elle est donc lue par pg_get_functiondef puis reecrite par substitution d un motif, avec RAISE si le motif est absent ou apparait plus d une fois (pas de no-op silencieux). Le motif est compare TEL QUEL, en MINUSCULES : le dernier definisseur de save_crm_actor dans ce manifeste est l etape 8z3 ../supabase/migrations/20260807124408_actor_prospects_documents.sql, dont le corps le porte en minuscules et EXACTEMENT une fois — migration_crm_module.sql, bien plus haut, ecrit le meme predicat en MAJUSCULES et ferait echouer le patch sur la branche « le bras a change de forme ». Passe aussi APRES 17c migration_crm_notes_probe.sql, dernier definisseur de current_user_can_write_crm_notes. AUCUN fichier de test versionne, cf. runbook 17j) =='
\ir migration_crm_write_requires_permission.sql

\echo '== 17k    migration_list_write_creator_only.sql  (§227 : ecrire une liste, c est son CREATEUR — pas « n importe quel role admin ». api.user_can_write_list portait le dernier exemplaire du motif ferme par 17j, meme defaut : une NON-NULLITE et non un seuil, si bien que team_lead (rang 10) pouvait modifier, partager, marquer envoyee ou SUPPRIMER la liste de n importe qui dans l ORG. Pas de permission inventee ici, deliberement : il n existe aucun droit « ecrire une liste » au catalogue et api.create_list n en demandait aucun — une liste est une SELECTION personnelle, la bonne question n est pas quel droit mais a qui elle appartient. SECOND BRAS ETROIT contre les listes ORPHELINES : object_list.created_by ne porte AUCUNE cle etrangere, au depart d un membre sa liste resterait inedittable par toute l ORG ; un administrateur d ORG de rang >= 30 peut donc reprendre une liste dont le createur n est PLUS membre actif, et seulement dans ce cas. COALESCE(..., FALSE) en tete : is_platform_superuser() rend NULL hors contexte HTTP et les appelants ecrivent IF NOT ... THEN RAISE, ou NOT NULL ne declenche PAS le RAISE — la garde deviendrait fail-OPEN. APRES L1 migration_object_list.sql, seul definisseur de la fonction. AUCUN fichier de test versionne, cf. runbook 17k) =='
\ir migration_list_write_creator_only.sql

\echo '== 17l NON DECLARE, DELIBEREMENT (2026-09-04) : migration_list_create_superuser_only.sql est APPLIQUEE EN PRODUCTION depuis le 2026-08-31 et reste absente de ce manifeste — sa declaration ferait ROUGIR la garde CI. Elle reserve api.create_list au superuser plateforme, or tests/test_object_list.sql (rejoue par le workflow APRES ce manifeste, etape « §211 E1 Listes module non-regression ») appelle api.create_list ligne 98 sous une persona authenticated dont app_user_profile.role vaut tourism_agent : is_platform_superuser() y est FALSE et l appel leverait 42501. Le test n est pas casse, il encode la regle produit d AVANT l arbitrage PO du 2026-08-31 — c est LUI qui doit etre repris (persona superuser pour la creation, ou assertion du refus), et ce geste sort du perimetre de la reparation de manifeste. TANT QUE CE TEST N EST PAS REPRIS, NE PAS DECLARER 17l ICI. Verifie le 2026-09-04 : test_object_list.sql est le SEUL test du depot qui appelle api.create_list ou api.user_can_write_list =='
