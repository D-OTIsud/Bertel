...
\echo '== 18b0  migration_test_org_facets.sql  (la profondeur PAR TYPE du corpus de test). 18b remplissait les tables COMMUNES et s arretait la : les 270 fiches n avaient AUCUNE ligne de facette. Un sentier sans distance, sans denivele, sans etape et sans trace ; une manifestation SANS DATE ; un hotel sans chambre ; un restaurant sans carte. Le corpus etait complet au sens du NOMBRE de fiches et vide au sens du METIER — et la garde de 18b ne le voyait pas, parce qu elle verifiait la profondeur COMMUNE, c est-a-dire exactement ce qui avait ete construit. Une garde qui n interroge que ce qu on a fait ne dit rien de ce qu on a oublie (meme motif que les 42 policies inlinees de 18a). Suit ref_facet_applicability a la lettre — 7 types (COM PCU PNA PRD PSV SPU VIL) n ont aucune facette et n en recoivent pas ; trg_assert_facet_applicable refuserait le reste. Piege releve : object_iti.open_status n accepte que 4 des 7 codes de ref_code_iti_open_status (not_managed, unknown, archived y sont REFUSES par le CHECK de la colonne). AVANT 18b, qui APPELLE cette fonction depuis seed_test_corpus — c est la seule facon que la remise a zero resseme aussi les facettes. =='
