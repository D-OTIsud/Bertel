...
\echo '== 17i-test garde permanente de l outbox e-mail et des pieces jointes, prouvee ROUGE avant application (P0004 A1 : les colonnes d outbox n existaient pas). A structure et §204 sur les quatre fonctions exposees, chaque NOT EXISTS double d une premisse de NON-VACUITE (service_role doit se voir dans la vue, sinon la garde est vraie parce que la vue est vide) / B la FENETRE de reclamation eprouvee DANS LES DEUX SENS - 5 min encore reclamee, 11 min re-reclamable : now() etant FIGE sur la transaction, un re-claim immediat reste vide MEME avec un TTL de zero, si bien qu une garde reduite a ce seul cas ne prouverait AUCUNE duree (verifie : le sabotage TTL=0 passe B2a et ne leve que sur B2b) ; acquittement en succes definitif (un second passage ne reecrit rien, garde email_sent_at IS NULL), acquittement en echec qui LEVE le claim et rend la ligne immediatement re-reclamable ; destinataire sans e-mail TERMINE sur place et jamais rendu, sans quoi il boucle claim/echec et bouche la file ; les deux RPC fermes a authenticated / C le predicat porte la meme regle que save_crm_task, la tache inconnue rendant false SOUS UNE PERSONA QUI PEUT ECRIRE (sous une persona sans droit, false ne prouverait rien) / D documents[] avec id = document_id, [] jamais null, et NON-REGRESSION du contrat 16z (assignees[], created_by_id/name) que le redeploiement de list_crm_tasks pourrait effacer. Non-vacuite prouvee par DEUX sabotages en transaction annulee, chacun levant sur SA garde (TTL 0 -> B2b, retrait du bloc no_recipient_email -> B5) =='
