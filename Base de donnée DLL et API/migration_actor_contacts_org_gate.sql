...
COMMENT ON FUNCTION api.can_read_actor_contacts(text) IS
  'SOURCE AUTORITAIRE (§208) de la règle « qui voit les coordonnées complètes d''un acteur » : superuser plateforme (lu dans app_user_profile.role — délibérément PAS api.is_platform_superuser(), dont le premier bras dirait TRUE à une clé service_role) OU membre d''une ORG publisher de la fiche (api.current_user_crm_object_ids). FALSE hors contexte HTTP et en service-role (auth.uid() NULL) : un export de PII est imputable à une personne. Forme PAR FICHE, appelée par api.export_actor_capabilities et par l''éditeur. TROIS autres formulations existent (mise à jour §213, 2026-08-09 ; « UNE seule » à l''origine, « DEUX » à la tâche 7) : la forme ensembliste du périmètre dans api.export_actor_contacts (duplication délibérée §204) ; le bras superuser de la cascade prestataire→fiche de api.list_selection_emails (migration_selection_emails.sql, §211 plié au régime §208) ; et le périmètre PAR ACTEUR de la colonne email de api.search_actors (migration_actor_links_editor.sql, §213), qui n''a aucun contexte objet et demande donc « cet acteur est-il rattaché à au moins une de mes fiches ? » — écart assumé et documenté sur place : ce bras n''applique ni visibility, ni validité temporelle, ni refus de consentement, contrairement à list_selection_emails. Faire évoluer les QUATRE ensemble.';
