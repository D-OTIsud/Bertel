-- migration_explorer_name_relevance.sql
-- Spec 2026-08-26 (docs/superpowers/specs/2026-08-26-explorer-recherche-pertinence-concordances-design.md)
--
-- DEUX correctifs de CLASSEMENT de la recherche Exploreur, sans changement de signature :
--
-- A1 — api.get_filtered_object_ids : BONUS NOM dans `relevance`.
--   Constat mesuré sur la prod : la fiche nommée EXACTEMENT « Le Jardin Créole » scorait
--   2.2577 contre 2.2395 pour une fiche ne portant ni « jardin » ni « créole » dans son nom
--   — l'écart était du bruit, et le second homonyme tombait 5e. Cause : ts_rank récompense
--   la DENSITÉ des termes dans le document, jamais l'EXACTITUDE du nom.
--   Étages désormais étanches PAR CONSTRUCTION (bonus espacés de 1.0 entier ET ts_rank
--   plafonné à 0.99 — ts_rank n'est pas borné à 1 en théorie : sans plafond un document
--   dense ferait sauter un étage) :
--       nom exact   [5,6)  >  préfixe [4,5)  >  contenu [3,4)
--     > plein texte  [2,3)  >  flou    [0,1]   (repli §197/§199, inchangé)
--   Le bonus ne s'applique QU'AU bras plein texte (la fiche a déjà matché). Coût nul : la
--   liste SELECT est évaluée APRÈS filtrage, sur les seules survivantes.
--   `position()` et non LIKE : aucun échappement de métacaractère à prévoir.
--   `name_normalized` NULL ⇒ tous les WHEN valent NULL ⇒ ELSE 0 (pas de COALESCE requis).
--
-- A2 — api.list_object_resources_filtered_page : émettre `relevance` PAR CARTE.
--   L'ORDER BY serveur classait déjà correctement, mais la clé n'était pas rendue : le front
--   recollait les pages de 7 buckets et retombait sur l'alphabétique (sortExplorerCards),
--   annulant le classement. Même mécanique d'attache positionnelle que `label_match`.
--   Toujours émis (0 sans terme) ⇒ sans recherche l'ordre historique est préservé.
--
-- CORPS COMPLET des deux fonctions, dérivé de migration_explorer_remplissage_filter.sql
-- (16r, §204) — vérifié IDENTIQUE au prosrc vif ligne à ligne avant dérivation (§213).
-- ⚠️ NE PAS dériver de api_views_functions.sql : ce fichier est resté PRÉ-§204 (aucune
-- occurrence de missing_essentials) ; le déployer effacerait le filtre Remplissage.
-- Créneau manifeste 16v — APRÈS 16r. NOTIFY pgrst non requis (signatures inchangées).
-- CI : tests/test_explorer_name_relevance.sql
    ...
    CASE
      ...
      ELSE (
        SELECT GREATEST(
          ...
          -- Étages étanches par construction — bonus espacés de 1.0 entier ET ts_rank
          -- plafonné à 0.99 (non borné à 1 en théorie : sans plafond, un document dense
          -- ferait sauter un étage et un vrai positif passerait derrière du bruit).
          -- position() plutôt que LIKE : aucun métacaractère à échapper.
          -- name_normalized NULL ⇒ tous les WHEN sont NULL ⇒ ELSE 0 (pas de COALESCE).
...
BEGIN
  ...
  decorated_data AS (
    ...
    -- `relevance` (spec 2026-08-26) suit le MÊME chemin d'attache et est TOUJOURS émis :
    -- le front recolle les pages de plusieurs buckets et n'a aucune autre clé pour
    -- restituer l'ordre serveur. Sans terme de recherche il vaut 0 partout ⇒ le tri
    -- alphabétique historique est préservé à l'identique.
    SELECT COALESCE(
      jsonb_agg(
        CASE
