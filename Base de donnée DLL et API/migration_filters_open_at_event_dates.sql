-- =====================================================================
-- migration_filters_open_at_event_dates.sql  (manifest step 16i; decision log §157)
-- Explorer : filtre « Ouvert à … » (instant arbitraire) + filtre Dates Événements.
--
-- BESOIN PO : « la liste des restaurants qui seront encore ouverts à 18 h alors
--   qu'il est 10 h » — impossible : le filtre open_now lisait UNIQUEMENT le cache
--   de l'instant présent (object.cached_is_open_now, cron ~15 min). Et le bucket
--   Événements n'avait AUCUN filtre de dates alors qu'object_fma porte
--   event_start_date/event_end_date (principe §150 : la surface de filtrage suit
--   le modèle, pas les données du moment).
--
-- DESIGN :
--   1. api.get_local_time_for_timezone(tz, p_at) — le résolveur d'heure locale
--      paramétré (même validation try/catch AT TIME ZONE que §37 — jamais de scan
--      pg_timezone_names). get_local_now_for_timezone devient son délégué à
--      CURRENT_TIMESTAMP (une seule implémentation).
--   2. internal.compute_open_status(p_at) — LE moteur d'ouverture (2 bras EXISTS
--      + tri-état §133 + « ouvert sans horaire » §93 + récurrence §92), paramétré.
--      Publiés uniquement (parité stricte avec le cache).
--   3. api.refresh_open_status() = écriture-cache de compute_open_status(now())
--      (comportement byte-équivalent ; source unique, plus de dérive possible
--      entre la pastille et le futur filtre).
--   4. api.get_filtered_object_ids : clé `open_at` (ISO timestamptz — moteur
--      évalué UNE FOIS dans un CTE, jamais en LATERAL par ligne, leçon §37 ;
--      match uniquement is_open = TRUE, le NULL tri-état n'est jamais matché,
--      invariant §133) + clé `event:{from,to}` (recouvrement de plage sur
--      object_fma ; la récurrence texte-libre n'est PAS évaluée — limite
--      documentée).
--
-- BLAST RADIUS : refresh_open_status (cron), get_local_now_for_timezone (2 appelants
--   internes), get_filtered_object_ids (Explorer + listes dynamiques + markers).
--   Signatures inchangées ⇒ pas de NOTIFY pgrst requis (clé jsonb additive).
-- IDEMPOTENT : CREATE OR REPLACE partout. FOLDED : schema_unified.sql (fns 1-3)
--   + api_views_functions.sql (fn 4) — fresh == live.
-- REVERSIBLE : git checkout des deux fichiers foldés + ré-application des
--   anciennes définitions (§133 pour refresh, §37 pour get_local_now).
-- POST-APPLY :
--   SELECT api.refresh_open_status();  -- parité immédiate (le cron le ferait)
-- =====================================================================
...
-- ── 4. get_filtered_object_ids (corps complet, extrait du source foldé) ──
