...
CREATE POLICY "canonical_ins_object_room_type_amenity" ON object_room_type_amenity FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM object_room_type rt WHERE rt.id = room_type_id AND api.user_can_write_object_canonical(rt.object_id)) OR EXISTS (SELECT 1 FROM object_room_type rt JOIN object o ON o.id = rt.object_id WHERE rt.id = room_type_id AND o.created_by = (select auth.uid())));
DROP POLICY IF EXISTS "canonical_upd_object_room_type_amenity" ON object_room_type_amenity;
CREATE POLICY "canonical_upd_object_room_type_amenity" ON object_room_type_amenity FOR UPDATE USING (EXISTS (SELECT 1 FROM object_room_type rt WHERE rt.id = room_type_id AND api.user_can_write_object_canonical(rt.object_id)) OR EXISTS (SELECT 1 FROM object_room_type rt JOIN object o ON o.id = rt.object_id WHERE rt.id = room_type_id AND o.created_by = (select auth.uid()))) WITH CHECK (EXISTS (SELECT 1 FROM object_room_type rt WHERE rt.id = room_type_id AND api.user_can_write_object_canonical(rt.object_id)) OR EXISTS (SELECT 1 FROM object_room_type rt JOIN object o ON o.id = rt.object_id WHERE rt.id = room_type_id AND o.created_by = (select auth.uid())));
