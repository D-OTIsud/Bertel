...
-- DEFERRED NO-OP: tourisme_handicap retired (V5 canonical: LBL_TOURISME_HANDICAP).
-- Left untouched — TST refactor pass required. Resolves to zero rows at execution time.
...
-- DEFERRED NO-OP: green_key retired (V5 canonical: LBL_CLEF_VERTE); value 'green_key' was never seeded.
-- Left untouched — TST refactor pass required. Resolves to zero rows at execution time.
INSERT INTO object_classification (object_id, scheme_id, value_id, created_at, updated_at)
SELECT o.id, cs.id, cv.id, NOW(), NOW()
FROM object o, ref_classification_scheme cs, ref_classification_value cv
...
-- DEFERRED NO-OP: tourisme_handicap retired (V5 canonical: LBL_TOURISME_HANDICAP); value 'moteur' ≠ seeded 'motor'.
-- Left untouched — TST refactor pass required. Resolves to zero rows at execution time.
INSERT INTO object_classification (object_id, scheme_id, value_id, created_at, updated_at)
SELECT o.id, cs.id, cv.id, NOW(), NOW()
FROM object o, ref_classification_scheme cs, ref_classification_value cv
...
WITH scheme_ids AS (
  ...
  VALUES
    ...
    -- DEFERRED NO-OP: tourisme_handicap retired (V5 canonical: LBL_TOURISME_HANDICAP).
    -- These i18n rows target values that will not exist. Resolves to zero UPDATE rows.
    -- Left untouched — TST refactor pass required.
...
-- B-3) Famille amenity accessibilite (via ref_code)
...
-- B-4) Équipements accessibilité (32 items)
WITH family AS (
  SELECT id FROM ref_code_amenity_family WHERE code = 'accessibilite' LIMIT 1
