...
--       iti_difficulty (1-5), iti_open_status (the four object_iti core statuses), iti_stage_kind
--   * migration_trail_referential.sql later extends iti_open_status with trail-only statuses;
--     this test owns the four Section 06 codes and must not reject those later additions.
...
BEGIN
  ...
  ASSERT (SELECT count(*) FROM ref_code
          WHERE domain = 'iti_open_status'
            AND code IN ('closed','open','partially_closed','warning')) = 4,
    'iti_open_status must contain the 4 Section 06 core codes';
  ASSERT (SELECT count(*) FROM ref_code WHERE domain = 'iti_stage_kind') = 8,
    'iti_stage_kind must have exactly 8 codes';

  -- The Section 06 subset must align with object_iti.open_status. The shared ref_code
  -- domain also contains statuses used only by trail_* after migration_trail_referential.sql.
  ASSERT ARRAY['closed','open','partially_closed','warning']::text[] <@
         (SELECT array_agg(code ORDER BY code) FROM ref_code WHERE domain = 'iti_open_status'),
    'iti_open_status must contain every object_iti.open_status CHECK value';
