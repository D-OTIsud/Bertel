...
-- Garde de api.get_object_workspace_permissions (agrégateur des 8 sondes).
--
-- Ce que la garde protège réellement :
--  1. l'attribut de sécurité — repasser en SECURITY DEFINER élargirait la
--     surface privilégiée pour rien (les 8 feuilles sont déjà DEFINER) ;
--  2. les grants — `anon` ne doit pas pouvoir l'exécuter ;
--  3. le contrat de clés — le front lit `canonical`/`enrichment`/`publish`/
--     `private_notes`/`owner`/`crm`/`org_admin`/`platform_superuser` ;
--  4. la PARITÉ avec les 8 sondes individuelles — c'est l'assertion non
...
BEGIN
  ...
  END IF;
  IF v_secdef THEN
    RAISE EXCEPTION 'get_object_workspace_permissions doit rester SECURITY INVOKER : les 8 feuilles sont deja DEFINER et gatent elles-memes';
  END IF;
  IF v_anon THEN
  ...
  -- 4 — parite avec les 8 sondes individuelles, sur un objet REEL
