...
// Dates de démo RELATIVES à aujourd'hui : le kanban ouvre sur une fenêtre glissante
// -15/+15 jours (16w). Des dates figées rendraient la démonstration vide dès la quinzaine
// suivante — un tableau vide se lit comme une panne, pas comme un filtre.
// Sert aux DEUX bases de la période (échéance ET création), d'où le nom générique.
// Offsets de CRÉATION choisis pour que la bascule Échéance→Création se VOIE en démo :
// task-3 (sans échéance, donc simple échappée sous « Échéance ») est créée il y a 40 jours
// et sort donc de la fenêtre -15/+15 dès qu'on bascule. Un jeu d'offsets tous dans la
// fenêtre ferait lire le sélecteur comme inerte.
const DEMO_DAY_MS = 86_400_000;
const demoDate = (offsetDays: number): string => new Date(Date.now() + offsetDays * DEMO_DAY_MS).toISOString();

export const mockCrmTasks: CrmTask[] = [
  // §66 — task-1 illustre une tâche LIÉE à une interaction encore ouverte (badge + prompt de clôture).
  // 16w — task-1 porte DEUX assignés (le cas multi de la maquette) ; task-3 est celle d'un
  // collègue (elle n'apparaît que sous « Toutes les personnes ») ET illustre un créateur
  // inconnu (« Créateur inconnu » sur la carte), comme les tâches nées d'un incident.
  { id: 'task-1', objectId: 'obj-1', objectName: 'Hotel Basalte & Lagon', actorId: 'actor-1', actorName: 'Mme Marie Hoarau', title: 'Rappeler le directeur', description: null, status: 'todo', priority: 'high', dueAt: demoDate(-2), createdAt: demoDate(-5), assignees: [{ userId: 'usr-local-marie', displayName: 'Marie D.' }, { userId: 'usr-local-jean', displayName: 'Jean P.' }], createdById: 'usr-local-marie', createdByName: 'Marie D.', ownerId: 'usr-local-marie', ownerName: 'Marie', relatedInteractionId: 'int-1', relatedInteractionSubject: 'Demande de visite guidée', relatedInteractionStatus: 'new', documents: [] },
  { id: 'task-2', objectId: 'obj-2', objectName: 'Le Comptoir des Epices', actorId: null, actorName: null, title: 'Valider le contrat photo', description: null, status: 'in_progress', priority: 'medium', dueAt: demoDate(1), createdAt: demoDate(-10), assignees: [{ userId: 'usr-local-marie', displayName: 'Marie D.' }], createdById: 'usr-local-jean', createdByName: 'Jean P.', ownerId: 'usr-local-marie', ownerName: 'Marie', relatedInteractionId: null, relatedInteractionSubject: null, relatedInteractionStatus: null, documents: [] },
  { id: 'task-3', objectId: 'obj-3', objectName: 'Sentier des Trois Cascades', actorId: null, actorName: null, title: 'Confirmer les horaires d hiver', description: null, status: 'done', priority: 'low', dueAt: null, createdAt: demoDate(-40), assignees: [{ userId: 'usr-local-luc', displayName: 'Luc T.' }], createdById: null, createdByName: null, ownerId: 'usr-local-luc', ownerName: 'Luc', relatedInteractionId: null, relatedInteractionSubject: null, relatedInteractionStatus: null, documents: [] },
];
...
export const mockCrmTimeline: CrmTimelinePage = {
  items: [
    { id: 'evt-1', actorId: 'actor-1', objectId: 'obj-1', objectName: 'Hotel Basalte & Lagon', interactionType: 'call', direction: 'outbound', status: 'new', subject: 'Appel de suivi', body: 'Besoin d une nouvelle photo facade.', occurredAt: '2026-06-11T11:12:00Z', actorName: 'M. Payet', topicCode: null, topicName: null, sentimentCode: 'positif', sentimentName: 'Positif', ownerName: 'Marie', source: 'bertel_ui', interlocutorEmail: null, resolvedAt: null, replies: [{ id: 'evt-1-r1', interactionType: 'note', body: 'Photo recue, a integrer.', occurredAt: '2026-06-11T14:00:00Z', createdAt: '2026-06-11T14:01:00Z', sentimentCode: null, sentimentName: null, ownerName: 'Marie', interlocutorEmail: null, source: 'bertel_ui' }] },
    { id: 'evt-2', actorId: 'actor-2', objectId: 'obj-2', objectName: 'Le Comptoir des Epices', interactionType: 'email', direction: 'outbound', status: 'resolved', subject: 'Validation tarifs 2026', body: 'Mail envoye pour validation des tarifs 2026.', occurredAt: '2026-06-11T09:40:00Z', actorName: null, topicCode: 'modification_infos_bdd', topicName: 'Modification infos BDD', sentimentCode: null, sentimentName: null, ownerName: 'Luc', source: 'bertel_ui', interlocutorEmail: null, resolvedAt: '2026-06-11T10:00:00Z', replies: [] },
  ],
  hasMore: false,
};
