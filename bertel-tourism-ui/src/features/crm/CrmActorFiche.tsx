...
//
// Mise en page deux colonnes (rectif PO §66+) :
//  - COLONNE GAUCHE (.crm-actor-grid__main) : les actions (« Nouvelle tâche » / « Nouvelle
//    interaction ») PUIS l'historique d'interactions (timeline + chips de contexte + fils).
//  - COLONNE DROITE (.crm-actor-grid__side, rail) : la carte acteur (avatar, nom, identité,
//    Coordonnées cliquables verticales, « Modifier ») PUIS les KPI compacts PUIS « Établissements
//    & rôles » PUIS « Sujets récurrents ».
...
export function CrmActorFiche({
  ...
  return (
    <div className="crm-body">
      ...
      {/* Deux colonnes (rectif PO §66+) : main (actions + timeline) à GAUCHE, rail (acteur + KPI +
          listes) à DROITE. Sur mobile la grille s'effondre en 1 colonne avec le rail (order: -1)
          AVANT la colonne main — voir styles.css .crm-actor-grid. */}
