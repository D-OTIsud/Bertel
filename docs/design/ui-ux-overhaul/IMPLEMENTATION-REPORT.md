...
## Phase 5 — Dashboard / CRM / secondaires ✅ (2026-06-23)
...
### 5.2 — Part B : modale d'interaction en 2 temps ✅ (`f06f67e`, 2026-06-23)
- **Fin du formulaire-dans-formulaire** : la case « Créer une tâche de suivi liée » imbriquée est retirée. Flux en **deux temps** : (1) Consigner l'interaction → état de confirmation « Interaction enregistrée », (2) affordance « + Ajouter une relance » qui révèle les champs (titre/échéance/assigné), enregistrée **séparément**. « Terminer » ferme sans relance.
- **Deux mutations distinctes** (interaction / tâche) ⇒ **idempotence naturelle** : un échec de relance ne re-crée jamais l'interaction (retry = tâche seule) ; contrat `saveCrmTask(relatedInteractionId)` inchangé. Pas de write-trap (relance désactivée-avec-raison si titre vide).
- **Tension documentée** : le §66 (décision PO 1) avait **choisi** la tâche liée imbriquée ; la maquette 5.2 la remplace par le flux 2-temps — option la plus conforme à la maquette, invariant write-path préservé.
- TDD : 13 specs (5 « établissement requis » inchangées + 8 réécrites) + `CrmActorFiche` réaligné (le modal reste ouvert, « Terminer » ferme). **CRM 235/235** ; build 0 ; tsc sans nouvelle erreur.

### Restant (différé avec raison)
- **Unification Team/RGPD sur le vocabulaire maison** : absorbée par la Phase 7.4 (Team) ; RGPD reste.
- **Pages stub honnêtes** (`EmptyState mode=coming-soon`) : Moderation/Audits/Publications déjà câblés (S12).
...
### 7.5 — Listes & référentiels (ref_code) ✅ (`aa894d2`, 2026-06-23, back-end LIVE)
- **Back-end** (`migration_ref_code_admin_rpcs.sql`, appliqué LIVE + foldé) : `ref_*` est admin-write RLS ⇒ écriture via 4 fonctions `SECURITY DEFINER` gated `api.is_platform_superuser()` (précédent `api.create_tag`) : `api.ref_code_domain_is_editable` (éditable = NON structurel : pas d'entrée registre taxonomie/hiérarchique/object_type), `api.rpc_upsert_ref_code` (crée code normalisé+**verrouillé** / édite libellé+i18n+position), `api.rpc_set_ref_code_active`, `api.rpc_reorder_ref_code`, + lecture `api.list_ref_code_domains`.
- **Gate FAIL-CLOSED** : `IS NOT TRUE` (pas `NOT`) — `is_platform_superuser()` peut renvoyer NULL hors contexte auth ; `IF NOT NULL` ne lèverait PAS (fail-open). Bug attrapé + corrigé avant la fin de passe.
- **Vérifié LIVE** (transaction-local super-admin + ROLLBACK forcé) : create / edit-code-verrouillé / deactivate / refus-domaine-structurel (42501) / **fail-closed sans super-admin**, 0 ligne de test persistée. Test CI `tests/test_ref_code_admin_rpcs.sql`.
...
- **Phase 5** ✅ **complète** (5.1 hiérarchie dashboard · **5.2 A+B+C** dé-modalisation acteur en tiroir + modale interaction 2-temps + états vides enseignants · 5.3 login + pages stub honnêtes · anti-pattern side-stripe)
