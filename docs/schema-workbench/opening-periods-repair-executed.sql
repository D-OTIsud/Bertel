-- =============================================================================
-- OPENING PERIODS — REPAIR EXECUTED  (phase 1: structural collapse)
-- =============================================================================
-- STATUS  : *** EXECUTED ON 2026-05-19 (test DB, single operator) ***
-- DATE    : 2026-05-19
-- AUTHOR  : schema-workbench audit
-- SOURCE  : copied from docs/schema-workbench/opening-periods-repair-draft.sql
--           ONLY differences vs draft:
--             - explicit "all-year-round" rule callout
--             - extra in-transaction assertion enforcing all_years=true on every
--               canonical row created by this repair (Step 8b)
--             - trailing ROLLBACK replaced with COMMIT
--             - status / header text
--
...
--
-- ALL-YEAR-ROUND RULE (preserved explicitly):
--   * The source CSV provides no seasonal `date_start` / `date_end` columns; it
--     carries `all_years = true` for every row. That is the canonical signal
--     for an open-all-year period and MUST be preserved during repair.
--   * In the target schema, `opening_period.date_start = NULL AND
--     opening_period.date_end = NULL AND opening_period.all_years = true` is a
--     VALID shape representing "ouverture toute l'année". It is NOT a missing
--     value to be discarded.
--   * Missing `opening_time_frame` rows on a `opening_time_period`, in contrast,
--     mean "hours unknown / incomplete", NOT "open all day". The repair never
--     synthesizes 00:00-24:00 frames to fill that gap.
--   * Therefore: this script keeps every legacy slot's horaires_id and copies
--     its real start_time / end_time / weekday associations into the new
--     canonical chain. It does NOT discard rows because of NULL seasonal dates.
--
-- BACKUPS (taken outside this script, before execution):
--   public.opening_period_bak_20260519_082607Z
--   public.opening_schedule_bak_20260519_082607Z
--   public.opening_time_period_bak_20260519_082607Z
--   public.opening_time_frame_bak_20260519_082607Z
--   public.opening_time_period_weekday_bak_20260519_082607Z
--
...
--              The Step 0 pre-assertion enforces 257-row precondition; running
--              again would raise `Expected 257 opening_period rows before repair`.
-- =============================================================================
...
--   ALL-YEAR-ROUND RULE: every row inserted here has `all_years = true` with
--   `date_start = NULL AND date_end = NULL`. This is the canonical
--   "ouverture toute l'année" shape — preserved deliberately, never discarded.
-- -----------------------------------------------------------------------------
...
SELECT
  ...
  NULL                        AS date_start,      -- source had none — all-year-round
  NULL                        AS date_end,        -- source had none — all-year-round
  np.new_source_period_id     AS source_period_id,
  true                        AS all_years,       -- source flagged all_years — preserved
  NULL                        AS name_i18n,
  jsonb_build_object(
    'legacy_repair', 'opening-periods-repair-2026-05-19',
    'source_sheet',  'form_j_h',
    'origin',        'berta_v2_csv_export',
    'all_year_round_preserved', true
  )                           AS extra,
  now(), now()
...
--   NOTE on hours-vs-period semantics: a missing `opening_time_frame` row here
--   would mean "hours unknown". We therefore COPY each legacy slot's real
--   start_time/end_time. We do NOT synthesize 00:00-24:00 frames for slots
...
-- -----------------------------------------------------------------------------
-- Step 8 — post-snapshot assertions (counts)
-- -----------------------------------------------------------------------------
DO $$
DECLARE
  v_n_period    int;
...
-- -----------------------------------------------------------------------------
-- Step 8b — all-year-round preservation assertion
--   Verify every canonical row created by this repair carries `all_years=true`
--   and NULL seasonal dates. If any row inserted by this repair had been
--   coerced into a different shape, this assertion would abort the transaction.
-- -----------------------------------------------------------------------------
DO $$
DECLARE
  v_repair_rows           int;
  v_repair_all_year_round int;
  v_repair_with_dates     int;
BEGIN
  SELECT COUNT(*) INTO v_repair_rows
    FROM public.opening_period
    WHERE extra ? 'legacy_repair'
      AND extra->>'legacy_repair' = 'opening-periods-repair-2026-05-19';

  SELECT COUNT(*) INTO v_repair_all_year_round
    FROM public.opening_period
    WHERE extra ? 'legacy_repair'
      AND extra->>'legacy_repair' = 'opening-periods-repair-2026-05-19'
      AND all_years = true
  ...
  SELECT COUNT(*) INTO v_repair_with_dates
    FROM public.opening_period
    WHERE extra ? 'legacy_repair'
      AND extra->>'legacy_repair' = 'opening-periods-repair-2026-05-19'
      AND (date_start IS NOT NULL OR date_end IS NOT NULL);

  RAISE NOTICE 'ALL-YEAR-ROUND: repair_rows=%, all_year_round=%, accidentally_dated=%',
               v_repair_rows, v_repair_all_year_round, v_repair_with_dates;

  IF v_repair_rows <> 191 THEN
    RAISE EXCEPTION 'Expected 191 rows tagged with legacy_repair, found %', v_repair_rows;
  END IF;
  IF v_repair_all_year_round <> 191 THEN
    RAISE EXCEPTION 'Expected all 191 repair rows to be all-year-round, found % such',
                    v_repair_all_year_round;
  END IF;
  IF v_repair_with_dates <> 0 THEN
    RAISE EXCEPTION 'Expected 0 repair rows with seasonal dates, found %', v_repair_with_dates;
  END IF;
END$$;

-- -----------------------------------------------------------------------------
-- Step 9 — finalize.
--   This is the EXECUTED variant. COMMIT is in place. Switching back to
--   ROLLBACK would undo all of the above.
-- -----------------------------------------------------------------------------
COMMIT;
-- ROLLBACK;  -- intentionally disabled in the executed variant
