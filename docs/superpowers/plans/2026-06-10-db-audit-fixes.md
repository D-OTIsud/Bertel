...
- [x] **Step 5: Commit:** **DONE — `aeb696c`** (8 files). `fix(db): object_fma per-command canonical write policies — editor FMA upsert was RLS-denied (manifest 8n, §47)`.
...
- [x] **Step 4: End-of-migration safety net.** **DONE** — the 58-table FOR-ALL leftover DO block is appended before `COMMIT`. Append a verification DO block inside the migration (fails the transaction if the restructure left a `FOR ALL` behind):
...
- [x] **Step 1: Write the test.** **DONE** (`test_write_policy_percommand.sql`; DO#1 structural 58-table coverage, DO#2 personas with the verified actor-link owner fixture + §40 X-path media). Also edited the 3 CI-blocker tests: `test_object_act_rls.sql` (per-command triple assert), `test_sp1_canonical_write_auth.sql` (object_description carve-out → per-command + vacuous-legacy note), `test_sp1b_canonical_coverage.sql` (verified unchanged-green). All green on live post-8o. Structure (full file, following the house template):
  ...
  4. `ROLLBACK;`.
- [x] **Step 2: Wiring:** **DONE** — workflow step (test_write_policy_percommand) + ci_fresh_apply 8o pair + runbook 8o entry + runbook Incremental-Update §47 caveat + READMEs `5l`. workflow test step (4-line shape) + (the migration's `\ir` goes in `ci_fresh_apply.sql` at position 8o) + runbook entry 8o + READMEs (letter `5l`). **Also add a caveat line to the runbook's "Incremental Update Order" section** (`docs/SQL_ROLLOUT_RUNBOOK.md` ~lines 65–76): *"After re-applying `rls_policies.sql` or `object_workspace_safe_write_rpcs.sql` to a deployed DB, ALWAYS re-run `migration_write_policy_percommand.sql` (8o) — those source files still create the retired `FOR ALL` families, and the incremental order runs migrations before them, so skipping this silently resurrects ~90 FOR ALL policies on live (P0.3 gotcha returns; `test_write_policy_percommand.sql` would flag it as live-vs-fresh drift)."* (Fresh-apply is safe without this — all FOR ALL creators sit at steps 6/7/8b/8c/8g, before 8o.) Runbook entry text:
...
> **DONE (all steps):** Pre-snapshot `93 FOR ALL / 3 per-command / 65 SELECT`; pre-probe GREEN (invariant behaviour); applied via MCP `write_policy_percommand` (self-verify DO block did not raise); post-snapshot `0 FOR ALL / 174 per-command / 65 SELECT`; post-probe GREEN; advisors: security byte-identical (0 new), performance shrank 1,599,913 → 1,114,698 chars (multiple-permissive warnings dropped, no new errors).
...
- [x] **Step 1: Confirm prerequisites on live:** **DONE** — anon + authenticated both EXECUTE `api.current_user_extended_object_ids()` (signature `()`); all 25 read policies confirmed flat `api.can_read_object(<col>)`. No GRANT needed. `SELECT has_function_privilege('anon','api.current_user_extended_object_ids()','EXECUTE') AS anon_ok, has_function_privilege('authenticated','api.current_user_extended_object_ids()','EXECUTE') AS auth_ok;` (verify the exact signature first with `\df`-equivalent: `SELECT p.oid::regprocedure FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='api' AND p.proname='current_user_extended_object_ids';`). If anon lacks EXECUTE, the migration must `GRANT EXECUTE … TO anon, authenticated` (it returns an empty set for anon — safe, same §35 logic as the `object` policy).
- [x] **Step 2: Write the migration.** **DONE** (`migration_child_read_gate_setbased.sql`; 25 §38-form blocks; object_relation→source_object_id, object_iti_section→parent_object_id, rest→object_id). Header: PREREQUISITES 8d (p03) + 8o (write policies out of SELECT) — manifest step **8p**; IDEMPOTENT (DROP IF EXISTS + CREATE); REVERSIBLE ("re-create the p03 form: `CREATE POLICY "read_<t>" … USING (api.can_read_object(<col>))`"). Template (×24, substituting table + column):
...
```
  ...
  > **DONE — EXPLAIN verified.** Before: `Seq Scan … Filter: api.can_read_object(object_id)`. After: `Filter: ((object_id = (hashed SubPlan 6).col1) OR (object_id = (hashed SubPlan 7).col1))` — SubPlan 6 = published set via `Index Only Scan` on the published index, SubPlan 7 = `current_user_extended_object_ids` (ProjectSet), both hashed/computed once; **no per-row `can_read_object`**. Anon byte-equivalence verified (published=1, draft=0).
- [x] **Step 5: Wiring** — **DONE** (runbook 8p + ci_fresh_apply 8p + READMEs `5m`; no new workflow step — read asserts went into the existing test_write_policy_percommand step). **Applied to live** (MCP `child_read_gate_setbased`, `{success:true}`). Post-EXPLAIN ✓ + advisors byte-identical to post-8o (0 new). Commit below. (runbook 8p + ci_fresh_apply + READMEs — README letter `5m`; workflow step ONLY if the dedicated test file was created, per Step 3), **apply to live** (`apply_migration` name `child_read_gate_setbased` — distinct from the existing §35 `test_read_gate_setbased` concern), post-EXPLAIN + advisors, **commit** (including the updated `test_p03_read_gate_coverage.sql` AND the `test_object_act_rls.sql` read-qual edit): `perf(db): §47 set-based child read gates (§38 form) on 25 flat read policies (manifest 8p)`.
...
- [x] **Step 4: WORKFLOW.md tracker** — **DONE** (marked the 2 rows DONE §47; added rows: editor §07 capacity filtering, nested-EXISTS read remainder, meeting_room/menu applicability breadth, audit smells watchlist). mark DONE (with date + §refs): the "Broad set-based child read policies + FOR ALL→per-command" row and the "RLS write-policy duplication owner_* vs workspace_*" row. Add new rows: editor §07 capacity-metric filtering by `ref_capacity_applicability` (deferred; unblocked by 13c seeds); nested-EXISTS read-policy set-based remainder (low value); `object_meeting_room`/`object_menu` applicability broadening when new business cases appear (seed-only INSERT); audit smells watchlist pointer (tag_link / XOR columns / cached aggregates / extra JSONB / opening tree — watch only, per the 2026-06-10 audit verdict, logged in §46).
- [x] **Step 5:** **graphify UNAVAILABLE** in this environment (`graphify: command not found`) — noted in the final report; the graph can be refreshed with `graphify update .` from the repo root on a machine that has it installed. Run `graphify update .` from the repo root (CLAUDE.md rule after code changes). If the command is unavailable in this environment, note it in the final report instead of failing.
- [x] **Step 6: Commit:** **DONE (plan tracker only).** NB: `CLAUDE.md`, `.claude/WORKFLOW.md`, `bertel-tourism-ui/claude_brief/lot1_mapping_decisions.md` are **gitignored** by the project (local-only briefs) — the §46/§47/invariant/tracker edits are saved to disk but not git-committed (consistent with all prior commits). `docs: §46/§47 decision log + CLAUDE.md invariants (applicability registry, per-command write policies)`.
