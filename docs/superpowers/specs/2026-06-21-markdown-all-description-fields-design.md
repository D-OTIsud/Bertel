...
- **Foldé** dans `schema_unified.sql` (fonctions, colonnes générées) **et** `api_views_functions.sql` (RPCs), ajouté au **manifeste/runbook** (`SQL_ROLLOUT_RUNBOOK.md`), prochain id **14w**, couvert par le **gate CI fresh-apply**.
...
- MAJ `lot1_mapping_decisions.md` (décision §106) + proposition d'invariant CLAUDE.md (§13).
...
**Modifiés backend** : `api_views_functions.sql` (toutes les RPCs de §3.2 + allow-lists + blocs `to_jsonb`), `schema_unified.sql` (fonctions + colonnes `*_normalized`), `SQL_ROLLOUT_RUNBOOK.md` (14w).
