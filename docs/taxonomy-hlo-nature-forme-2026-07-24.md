...
## 4. Pré-ventilation des `gite_villa` : 176 (= 178 − 2 fiches du lot nature §2, Villa Ti MoOn et Gîte Là-Haut)

Heuristique **unifiée en une passe** (celle que le script de recodage implémentera) : mots-clés du nom OU de la description canonique, priorité maison/villa > appartement > bungalow/chalet > gîte générique (`bedrooms` vide sur le lot — locatifs entiers §64 ; la capacité, présente sur 175, n'est pas discriminante pour la forme).

| Cible | Total |
|---|---|
| `maison` « Maison / villa » | **150** |
| `appartement` | **2** |
| rejoignent le pool du split bungalow/chalet (§5) | **3** |
| restent sur `location_saisonniere` (gîte générique — nature connue, forme non) | **5** |
| **Arbitrage PO** | **16** |

La fusion de la feuille cible « Maison / villa » absorbe 150/176 — la distinction villa↔maison n'a plus besoin d'être arbitrée.

**Les 16 en arbitrage PO** (aucun signal nom ni description) :
...
| HLORUN00000000VK | Au Grand R' | sans signal |
...
| HLORUN00000000YF | La Ferme des Pitayas | sans signal |
...
Défaut proposé pour ces 16 : **rester sur le nœud de nature** « Meublé de tourisme / gîte » (la nature est connue via Berta « Location saisonnière » ; la forme attendra) — conforme à la règle « jamais la forme ne force une décision ».
...
| HLORUN00000001B5 | L'Océan de Brilune | gite_villa | auto → `maison` (signal description) |
| HLORUN00000001B8 | L'Or du Temps | gite_villa | arbitrage (déjà dans les 16 du §4) |
...
| gite_villa sans signal (§4) | 16 | forme inconnue — défaut proposé : rester sur nature |
| bungalow_chalet sans signal (§5) | 10 | bungalow vs chalet vs mobil-home |
| Sans provenance Berta (§6) | 0 en plus | L'Or du Temps déjà compté §4 ; L'Océan de Brilune auto-classée |
| Micro-feuilles (§3) | 3 questions | gite_rural (garder/fondre) · cottage→maison ? · rez_de_chaussée→appartement ? |

**40 arbitrages nominatifs + 3 questions de structure** (sans double compte) ; **~196 fiches recodées automatiquement** : 157 depuis gite_villa (150 maison/villa + 2 appartement + 5 restent-sur-nature) + 39 du split (22 chalet + 17 bungalow) + jusqu'à 3 transferts gite_villa→split re-testés.
...
JOIN object_taxonomy ot ON ot.object_id = o.id AND ot.domain = 'taxonomy_hlo'
  -- COALESCE obligatoire : NULL NOT IN (...) vaut NULL, pas TRUE — une source NULL serait silencieusement exemptée
  AND COALESCE(ot.source, '') NOT IN ('taxonomy_audit_lot_c_20260717', 'taxonomy_nature_forme_arbitrage_20260724')  -- sessions PO uniquement
...
4. **Recodage** : ventilation auto (~196) + arbitrages PO — **deux sources distinctes** : `taxonomy_nature_forme_20260724` pour les recodages AUTOMATIQUES (qui restent contrôlés par la garde §9) et `taxonomy_nature_forme_arbitrage_20260724` UNIQUEMENT pour les décisions PO (exemptées de la garde) ; `note` par lot ; gardes fail-closed sur les comptes (patron §187).
